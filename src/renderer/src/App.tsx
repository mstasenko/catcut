...
async function renderSvg(overlay: ImageOverlay): Promise<string> {
  const image = await loadedImage(await window.replaycat.getSvgDataUrl(overlay.path))
...
async function exportSession(session: EditSession, outputPath: string): Promise<void> {
  ...
  await window.replaycat.exportVideo({
...
function reportExportFailure(error: unknown): void {
  ...
  useEditorStore.setState({ error: 'ReplayCat could not export this video. Check the destination and try again.' })
}
...
export default function App(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    const removeJobListener = window.replaycat.onJobProgress(store.setJob)
    const removeOpenListener = window.replaycat.onOpenPath((path) => void store.loadVideo(path))
    const removeResetListener = window.replaycat.onResetProject(() => {
      if (window.confirm('Reset the current project and forget its saved state?')) void store.resetProject()
    })
    const removeSaveListener = window.replaycat.onSaveRequest(async () => {
      const current = useEditorStore.getState()
      if (current.session) {
        await window.replaycat.saveSession(savedSession(current.session, current.history, current.future))
      }
    })
    return () => {
      removeJobListener()
  ...
  const runExport = async (): Promise<void> => {
    ...
    const outputPath = await window.replaycat.chooseExportPath(exportName(primarySource(session).metadata.name))
  ...
  const dropVideo = async (event: React.DragEvent): Promise<void> => {
    ...
    if (!file) return
    await store.loadVideo(await window.replaycat.getDroppedPath(file))
  }
