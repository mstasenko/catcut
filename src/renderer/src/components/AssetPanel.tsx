...
export function AssetPanel(props: AssetPanelProps): React.JSX.Element {
  ...
  const togglePreview = async (asset: AssetItem): Promise<void> => {
    ...
    try {
      preview.current?.pause()
      const audio = new Audio(await window.replaycat.getPathUrl(asset.path))
      ...
      preview.current = null
      setPreviewing(null)
      props.onError('ReplayCat could not preview this audio.')
    }
  }
  ...
  return (
    <section className="asset-panel category-menu">
      ...
      <div className="asset-list">
        {visible.length === 0 && <p className="empty-note">No media here. Place the meme folder next to ReplayCat.</p>}
        {visible.map((asset) => (
          <AssetRow
            key={asset.id}
            asset={asset}
