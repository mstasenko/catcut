...
export async function installDesktopIntegration(options: DesktopIntegrationOptions): Promise<void> {
  ...
  await Promise.all([
    copyFile(join(options.resourcesPath, 'resources', 'replaycat-icon.png'), join(iconDirectory, 'replaycat.png')),
    writeWhenChanged(join(applicationsDirectory, 'replaycat.desktop'), createDesktopEntry(options.appImagePath))
  ])
}
