...
export async function probeMedia(path: string): Promise<MediaMetadata> {
  ...
  return {
    ...
    duration: firstPositiveNumber(result.format.duration, video.duration),
...
export async function probeAsset(path: string): Promise<AssetMetadata> {
  ...
  return {
    ...
    width: firstPositiveNumber(video?.width),
    height: firstPositiveNumber(video?.height),
    hasAudio: Boolean(audio)
  }
}
...
export async function createProxy(metadata: MediaMetadata): Promise<{
  ...
  const cacheDirectory = join(cacheRoot, 'replaycat', 'proxies')
