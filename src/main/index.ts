...
protocol.registerSchemesAsPrivileged([
  {
    scheme: 'replaycat-media',
    privileges: { standard: true, secure: true, supportFetchAPI: true, stream: true }
  }
])

app.setName('ReplayCat')
app.setDesktopName('replaycat.desktop')
app.commandLine.appendSwitch('ozone-platform', 'wayland')
app.commandLine.appendSwitch('enable-features', 'AcceleratedVideoDecodeLinuxZeroCopyGL')
const headlessTest = process.env.REPLAYCAT_HEADLESS_TEST === '1'
if (headlessTest) {
...
async function gpuDiagnostics(): Promise<GpuDiagnostics> {
  ...
  const forcedOff = process.env.REPLAYCAT_E2E_GPU_OFF === '1'
  return {
...
function createWindow(paths: SessionPathRegistry): BrowserWindow {
  const windowSize = initialWindowSize(process.env.REPLAYCAT_E2E_COMPACT === '1')
  const windowIcon = app.isPackaged
    ? join(process.resourcesPath, 'resources', 'replaycat-icon.png')
    : join(__dirname, '../../src/replaycat-icon.png')
  ...
  Menu.setApplicationMenu(Menu.buildFromTemplate([{
    label: 'Project',
    submenu: [
      { label: `ReplayCat ${packageMetadata.version}`, enabled: false },
      { type: 'separator' },
...
function registerIpc(directories: DialogDirectories, paths: SessionPathRegistry): void {
  ipcMain.handle('dialog:open-video', async () => {
    const testSelection = process.env.REPLAYCAT_E2E_VIDEO
  ...
  ipcMain.handle('dialog:open-media', async () => {
    const testSelection = process.env.REPLAYCAT_E2E_MEDIA
  ...
  ipcMain.handle('dialog:export-path', async (_event, value: unknown) => {
    ...
    if (process.env.REPLAYCAT_E2E_OUTPUT) return paths.allowWrite(process.env.REPLAYCAT_E2E_OUTPUT)
...
async function startApplication(): Promise<void> {
  if (process.env.XDG_SESSION_TYPE !== 'wayland' || !process.env.WAYLAND_DISPLAY) {
    dialog.showErrorBox(
      'ReplayCat requires GNOME Wayland',
      'Start ReplayCat from an Ubuntu 26 GNOME Wayland desktop session.'
    )
    app.quit()
    return
  }
  ...
  protocol.handle('replaycat-media', (request) => mediaResponse(request, (path) => paths.canRead(path)))
