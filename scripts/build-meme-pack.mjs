...
const archivePath = join(distRoot, 'replaycat-media-pack.zip')
...
async function download(asset) {
  ...
  for (let attempt = 1; attempt <= 4; attempt += 1) {
    const response = await fetch(asset.url, {
      headers: { 'User-Agent': 'ReplayCat media pack builder (https://github.com/mstasenko/replay-cat)' }
    })
    if (response.ok) {
...
function addImage(asset, cached, manifest) {
  ...
  if (resizeWidth) {
    ...
    manifest.assets.push(manifestEntry(asset, relativePath, `Resized to at most ${resizeWidth}px wide by ReplayCat`))
    return
  }
...
function addAnimation(asset, cached, manifest) {
  ...
  if (extension(asset) === '.gif') {
    ...
    manifest.assets.push(manifestEntry(asset, relativePath, 'Format-converted from the source GIF by ReplayCat'))
    return
  }
...
const manifest = {
  name: 'ReplayCat Open Media Pack',
