...
async function launch(
  ...
  const app = await electron.launch({
    args: [main, input],
    env: e2eEnvironment({ ...(output ? { REPLAYCAT_E2E_OUTPUT: output } : {}), ...overrides })
  })
...
test('plays from the point selected on the timeline', async () => {
  const directory = temporaryDirectory('replaycat-seek-')
...
test('moves five seconds with arrow keys while paused or playing', async () => {
  const directory = temporaryDirectory('replaycat-arrows-')
  const input = syntheticVideo(directory, true)
  const app = await launch(input)
  const window = await app.firstWindow()
  const preview = window.locator('.camera-layer > video:not(.preview-transition-previous)')
...
test('keeps timeline and zoom controls on the transport row', async () => {
  const directory = temporaryDirectory('replaycat-zoom-')
  const input = syntheticVideo(directory, true)
  const app = await launch(input)
  const window = await app.firstWindow()
...
test('shows the source frame under the timeline pointer', async () => {
  const directory = temporaryDirectory('replaycat-hover-preview-')
  const input = syntheticVideo(directory, true)
  const app = await launch(input)
  const window = await app.firstWindow()
  const timeline = window.locator('.timeline')
...
test('highlights and cuts the chosen side of one cut point without a popup', async () => {
  const directory = temporaryDirectory('replaycat-one-point-')
  const input = syntheticVideo(directory, true)
  const app = await launch(input)
  const window = await app.firstWindow()
...
test('supports more than two cut points and selects the partition at the playhead', async () => {
  const directory = temporaryDirectory('replaycat-two-points-')
  const input = syntheticVideo(directory, true)
  const app = await launch(input)
  const window = await app.firstWindow()
...
test('keeps the preview healthy after repeated editing operations', async () => {
  const directory = temporaryDirectory('replaycat-edit-')
  const input = syntheticVideo(directory, true)
  const app = await launch(input)
  const window = await app.firstWindow()
  const preview = window.locator('.camera-layer > video:not(.preview-transition-previous)')
...
test('renders added text into the exported video', async () => {
  const directory = temporaryDirectory('replaycat-text-')
  const input = syntheticVideo(directory, true)
...
test('renders SVG library images into the exported video', async () => {
  const directory = temporaryDirectory('replaycat-svg-')
  const input = syntheticVideo(directory, true)
  const output = join(directory, 'output.mp4')
  const app = await launch(input, output)
  const window = await app.firstWindow()
...
test('exports WebM video audio together with an OGG effect', async () => {
  const directory = temporaryDirectory('replaycat-media-export-')
  const input = syntheticVideo(directory, true)
  const output = join(directory, 'output.mp4')
  const app = await launch(input, output)
  const window = await app.firstWindow()
...
test('prepares, previews, and exports an external GIF', async () => {
  const directory = temporaryDirectory('replaycat-gif-')
  const input = syntheticVideo(directory, true)
  ...
  const app = await launch(input, output, { REPLAYCAT_E2E_MEDIA: gif })
  const window = await app.firstWindow()
...
test('blocks editing and removes partial output when export is cancelled', async () => {
  const directory = temporaryDirectory('replaycat-cancel-')
  ...
  await expect(window.getByText('ReplayCat could not export this video. Check the destination and try again.')).toHaveCount(0)
})

test('ripple-inserts and exports a second main-timeline video', async () => {
  test.setTimeout(60_000)
  const directory = temporaryDirectory('replaycat-insert-')
  ...
  const app = await launch(first, output, { REPLAYCAT_E2E_VIDEO: second })
  const window = await app.firstWindow()
...
test('opens and exports a centered vertical Short project', async () => {
  const directory = temporaryDirectory('replaycat-short-')
  const input = syntheticVideo(directory, true)
  const output = join(directory, 'short.mp4')
  const app = await launch(input, output, { REPLAYCAT_E2E_VIDEO: input })
  const window = await app.firstWindow()
...
test('shows the cropped Short frame and active meme overlay in the timeline preview', async () => {
  const directory = temporaryDirectory('replaycat-short-hover-')
  const input = syntheticVideo(directory, true)
  const app = await launch(input, undefined, { REPLAYCAT_E2E_VIDEO: input })
  const window = await app.firstWindow()
  await click(window.getByRole('button', { name: 'Open Short', exact: true }))
  await click(window.getByRole('button', { name: 'Images', exact: true }))
...
test('restores normal-close state and Reset project forgets it', async () => {
  const directory = temporaryDirectory('replaycat-restore-')
  const input = syntheticVideo(directory, true)
  ...
  })).toEqual(['ReplayCat 0.1.0', 'Reset project'])
