...
test('one click inserts, marks, exports, and removes a half-speed Replay', async () => {
  ...
  const directory = mkdtempSync(join(tmpdir(), 'catcut-v3-replay-'))
  const input = syntheticVideo(directory)
  const output = join(directory, 'replay.mp4')
  const app = await electron.launch({ args: [main, input], env: e2eEnvironment({ CATCUT_E2E_OUTPUT: output, CATCUT_E2E_COMPACT: '1' }) })
  try {
...
test('text Pop previews from its start and exports through the local bitmap path', async () => {
  test.setTimeout(90_000)
  const directory = mkdtempSync(join(tmpdir(), 'catcut-v3-text-'))
  const input = syntheticVideo(directory, true)
  const output = join(directory, 'text.mp4')
  const app = await electron.launch({ args: [main, input], env: e2eEnvironment({ CATCUT_E2E_OUTPUT: output }) })
  try {
    const page = await app.firstWindow()
    await dismissHardwareWarningIfNeeded(page)
...
test('audio controls preserve 200 percent volume without native-volume errors', async () => {
  test.setTimeout(90_000)
  const directory = mkdtempSync(join(tmpdir(), 'catcut-v4-audio-'))
  const input = syntheticVideo(directory, true)
  const overlay = sound(directory)
  const output = join(directory, 'audio.mp4')
  const app = await electron.launch({ args: [main, input], env: e2eEnvironment({ CATCUT_E2E_MEDIA: overlay, CATCUT_E2E_OUTPUT: output }) })
  const errors: string[] = []
  try {
    const page = await app.firstWindow()
...
test('video audio settings follow Include audio and retain their values', async () => {
  const directory = mkdtempSync(join(tmpdir(), 'catcut-v4-video-audio-'))
  const input = syntheticVideo(directory, true)
  const app = await electron.launch({ args: [main, input], env: e2eEnvironment({ CATCUT_E2E_MEDIA: input }) })
  try {
    const page = await app.firstWindow()
    await dismissHardwareWarningIfNeeded(page)
    await click(page.getByRole('button', { name: 'New', exact: true }))
