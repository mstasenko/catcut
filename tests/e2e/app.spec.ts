...
test('launches as a native Wayland editor', async () => {
  ...
  expect(await app.evaluate(({ app }) => app.getName())).toBe('CatCut')
...
test('opens, edits, and exports real media', async () => {
  const directory = mkdtempSync(join(tmpdir(), 'catcut-e2e-'))
  ...
  const app = await electron.launch({
    args: [main, input],
    env: e2eEnvironment({ CATCUT_E2E_OUTPUT: output })
  })
  const window = await app.firstWindow()
  captureDiagnostics(window)
...
test('scrolls and previews visual assets before adding one', async () => {
  const directory = mkdtempSync(join(tmpdir(), 'catcut-picture-'))
  const input = join(directory, 'input.mp4')
  execFileSync(ffmpeg, [
    '-hide_banner', '-loglevel', 'error', '-y',
  ...
  const app = await electron.launch({
    args: [main, input],
    env: e2eEnvironment({ CATCUT_E2E_COMPACT: '1' })
  })
  const window = await app.firstWindow()
...
test('shows a plain warning when hardware acceleration is unavailable', async () => {
  const app = await electron.launch({
    args: [main],
    env: e2eEnvironment({ CATCUT_E2E_GPU_OFF: '1' })
  })
  const window = await app.firstWindow()
...
test('seeks and plays a synthetic twenty-minute video', async () => {
  const directory = mkdtempSync(join(tmpdir(), 'catcut-long-'))
